...
class JokulO2O extends PaymentModule
{
	...
	public function uninstall()
	{
		if (
			...
			Configuration::deleteByName('set_sub_account_sub_account_o2o');
			Configuration::deleteByName('set_sub_account_data_o2o');
			...
			Configuration::deleteByName('LIST_BANK_o2o');
	...
	private function _postValidation()
	{
		if (Tools::isSubmit('btnSubmit')) {
			...
			if (Tools::getValue('set_sub_account_sub_account_o2o')){
				if (!Tools::getValue('set_sub_account_data_o2o'))
	...
	private function _postProcess()
	{
		if (Tools::isSubmit('btnSubmit')) {
			...
			Configuration::updateValue('set_sub_account_sub_account_o2o', 				trim(Tools::getValue('set_sub_account_sub_account_o2o')));
			Configuration::updateValue('set_sub_account_data_o2o', 						trim(Tools::getValue('set_sub_account_data_o2o')));
		} else if (Tools::isSubmit('btnSubmitUpdate')) {
			$listContent = array();
			$listContent = Tools::safeOutput(Tools::getValue('LIST_BANK_o2o', Configuration::get('LIST_BANK_o2o')));
			$trimspace = preg_replace('/\s+/', '', $listContent);
			...
			Configuration::updateValue('LIST_BANK_o2o', json_encode($listDataBank));
		} else if (Tools::isSubmit('btnSubmitAdd')) {
			$listContent = array();
			$listContent = Tools::safeOutput(Tools::getValue('LIST_BANK_o2o', Configuration::get('LIST_BANK_o2o')));
			$trimspace = preg_replace('/\s+/', '', $listContent);
			...
			if (count($listDataBank) > 0) {
				...
				Configuration::updateValue('LIST_BANK_o2o', json_encode($listContentAdd));
			}
		}
		Tools::redirectAdmin(
	...
	public function renderForm()
	{
		...
		$payment_sub_account = [
			[
				'id_option_sub_account'	 => 'sub_account_o2o',
        ...
        <script type="text/javascript">
            jQuery(document).ready(function($) {
				checkbox_sac_select();
                $('#set_sub_account_sub_account_o2o').click(function() {
                    checkbox_sac_select();
                })

				function checkbox_sac_select() {
                    if($('#set_sub_account_sub_account_o2o').is(':checked')) {
                        $('#set_sub_account_data_o2o').fadeIn();
						$('#label_set_sub_account_data').fadeIn();
						$('#label_desc_sub_account_data').fadeIn();
                        $('#set_sub_account_sub_account_o2o').prop('required',true);
                    } else {
						$('#set_sub_account_data_o2o').val("");
						$('#set_sub_account_data_o2o').fadeOut();
						$('#label_set_sub_account_data').fadeOut();
						$('#label_desc_sub_account_data').fadeOut();
                        $('#set_sub_account_sub_account_o2o').prop('required',false);
                    }
                }; 
            })
        </script>
	...
	public function getConfigFieldsValues()
	{
		return array(
			...
			'set_sub_account_sub_account_o2o'	=> Tools::safeOutput(Tools::getValue('set_sub_account_sub_account_o2o', Configuration::get('set_sub_account_sub_account_o2o'))),
			'set_sub_account_data_o2o'			=> Tools::safeOutput(Tools::getValue('set_sub_account_data_o2o', Configuration::get('set_sub_account_data_o2o'))),
			'notification_url' 					=> Tools::getHttpHost(true). __PS_BASE_URI__ . 'modules/jokulo2o/request.php?task=notify'
		);
	}
